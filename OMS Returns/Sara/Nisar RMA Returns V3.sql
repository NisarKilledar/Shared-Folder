select count(distinct order_no) sales_orders, count(distinct ret_order_no) return_orders, sum(ret_units) returned_units, sum(demand) return_demand
from(
select distinct order_no, ret_order_no, ret_line_key, ret_units, ret_value, ret_discount, (ret_value-ret_discount) demand
from(
select distinct 
     so.order_line_key, so.order_no, so.order_date, so.original_ordered_qty, 
            ...
            ordhead.order_no ret_order_no, ordhead.order_date ret_date, ordline.order_line_key ret_line_key,  ordline.original_ordered_qty ret_org_qty, 
...
inner join (
    ...
    from (select a.order_line_Key, a.status, os.status_name 
            ...
            inner join omsprd.yfs_status os on a.status = os.status
                    ...
                    --and a.status = '1000.01'
                    --and a.status = '1000.02'
                    --and a.status = '1100'
                    --and a.status = '3900'
                    and a.status <> '1000'
        )
